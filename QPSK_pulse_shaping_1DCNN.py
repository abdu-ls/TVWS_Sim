...
# --- QUICK TEST SETTINGS (Runs in ~15-25 mins) ---
SNR_RANGE = np.array([-15, -10, -5, 0, 5, 10])
N_MC = 2000
N_MC_CALIB = 500                    # NEW: separate calibration trials (avoid threshold leakage)
SENSING_SAMPLES = 1024
SEQUENCE_LENGTH = 64                # REDUCED from 256: less "free" averaging gain for ED,
                                     # so any DNN advantage isn't just masked by ED's processing gain
SAMPLES_PER_SYMBOL = 4              # NEW: QPSK pulse-shaping factor -> creates temporal
                                     # correlation structure that ED cannot see but a CNN can
BATCH_SIZE = 64
EPOCHS = 20
DRL_EPISODES = 200
N_EVAL_EPISODES = 20
...
# ==============================================================================
def generate_qpsk_signal(n_symbols, sps):
    """Generates a QPSK waveform with rectangular pulse shaping.

    Repeating each symbol 'sps' times creates strong sample-to-sample
    correlation within a symbol period. A pure Energy Detector only looks
    at aggregate power (mean |y|^2) and is blind to this correlation, but
    a CNN can learn a matched-filter/correlator-like operation that
    exploits it -- this is what gives a learned detector real headroom
    over ED, as opposed to the previous version where the PU "signal"
    was itself just white Gaussian noise (making ED already optimal).
    """
...
def generate_received_signal(snr_db, n_samples, pu_state, sps=SAMPLES_PER_SYMBOL):
    """Generates received signal with block Rayleigh fading and AWGN.

    CHANGES vs. original:
    - H1 signal is now a structured QPSK waveform (not i.i.d. complex
      Gaussian), so H1 is no longer just "Gaussian with higher variance."
    - Fading 'h' is now a single tap per sensing window (block fading),
      matching the standard assumption that coherence time exceeds the
      sensing duration, instead of an independent draw per sample.
    """
    noise = (np.random.randn(n_samples) + 1j * np.random.randn(n_samples)) / np.sqrt(2)
    snr_linear = 10 ** (snr_db / 10)

    if pu_state == 1:  # H1: PU Present
        h = (np.random.randn() + 1j * np.random.randn()) / np.sqrt(2)  # block fading (one tap)
        n_symbols = int(np.ceil(n_samples / sps)) + 1
        s = generate_qpsk_signal(n_symbols, sps)[:n_samples]
...
# ==============================================================================
def calibrate_ed_threshold(X_calib_np, Y_calib_np, target_pf):
    """NEW: threshold is now fit on a held-out CALIBRATION set, not the
    same set used to report Pd/Pf. Avoids the previous 'oracle' leakage
    where Pf was ~= target_pf by construction on the reporting set."""
    energy = np.mean(X_calib_np ** 2, axis=(1, 2))
    h0_energy = energy[Y_calib_np.squeeze() == 0]
    return np.percentile(h0_energy, (1 - target_pf) * 100)


def evaluate_ed(threshold, X_np, Y_np):
...
class DNNSensor(nn.Module):
    """UPGRADED: 1D-CNN sensor (was a flatten+MLP).

    A flatten+MLP treats the sequence as an unordered feature vector and
    has to relearn positional/temporal structure from scratch with no
    inductive bias for it. A small 1D CNN, by contrast, has convolution
    kernels that naturally slide across time and can pick up the
    within-symbol correlation created by pulse shaping (matched-filter-
    like behavior) -- this is the actual mechanism that should let it
    beat plain Energy Detection on the structured signal above.
    """
    ...
    def forward(self, x):
        # x: (batch, seq_len, 2) -> Conv1d wants (batch, channels, seq_len)
        x = x.permute(0, 2, 1)
        x = self.conv(x)
        return self.head(x)
...
def calibrate_dnn_threshold(model, X_calib, Y_calib, target_pf):
    """NEW: calibrate on a held-out set (same fix as ED)."""
    model.eval()
    with torch.no_grad():
        preds = model(X_calib.to(DEVICE)).cpu().squeeze()
...
def evaluate_dnn(model, threshold, X_np, Y_np):
    model.eval()
    ...
    del dataset, dataloader, predictions, labels
    if DEVICE.type == 'cuda': torch.cuda.empty_cache()
    gc.collect()
    return Pd, Pf

# ==============================================================================
# 5. PHASE 2: OCCUPANCY PREDICTION (LSTM)  [unchanged]
# ==============================================================================
...
# ==============================================================================
# 6. PHASE 3: DYNAMIC SPECTRUM ACCESS (DQN vs DDQN)  [unchanged]
# ==============================================================================
...
def evaluate_access_throughput(agent, snr_db):
    ...
    capacity = 8 * np.log2(1 + 10 ** (snr_db / 10))
    total_throughput = 0
...
def run_simulation():
    ...
    for snr in SNR_RANGE:
        ...
        # Separate calibration set (threshold fitting) from eval set (Pd/Pf reporting)
        X_calib, Y_calib = generate_sensing_dataset(snr, N_MC_CALIB, SENSING_SAMPLES, SEQUENCE_LENGTH)
        X_eval, Y_eval = generate_sensing_dataset(snr, N_MC, SENSING_SAMPLES, SEQUENCE_LENGTH)

        # ED
        thr_ed = calibrate_ed_threshold(X_calib.numpy(), Y_calib.numpy(), TARGET_PF)
        pd, pf = evaluate_ed(thr_ed, X_eval.numpy(), Y_eval.numpy())
        ed_pd.append(pd); ed_pf.append(pf)
        print(f"    ED  -> Pd: {pd:.4f}, Pf: {pf:.4f}")

        # DNN
        model = train_dnn_sensor(snr)
        thr_dnn = calibrate_dnn_threshold(model, X_calib, Y_calib, TARGET_PF)
        pd, pf = evaluate_dnn(model, thr_dnn, X_eval, Y_eval)
        dnn_pd.append(pd); dnn_pf.append(pf)
        print(f"    DNN -> Pd: {pd:.4f}, Pf: {pf:.4f}")

        del X_calib, Y_calib, X_eval, Y_eval
        gc.collect()
    ...
    ax1.plot(SNR_RANGE, dnn_pd, 'r-s', label='DNN Sensor (CNN)')
    ...
    ax2.plot(SNR_RANGE, dnn_pf, 'r-s', label='DNN Sensor (CNN)')
